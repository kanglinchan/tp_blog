...
CREATE  TABLE IF NOT EXISTS `blog`.`blog_user` (
  `id` INT UNSIGNED NOT NULL COMMENT '用户ID（唯一识别号）' ,
  `username` VARCHAR(20) NOT NULL COMMENT '用户名' ,
  `password` VARCHAR(32) NOT NULL COMMENT '密码' ,
  `email` VARCHAR(100) NOT NULL COMMENT '用户邮箱' ,
  `create_time` TIMESTAMP NOT NULL COMMENT '创建账户时间' ,
  `logintime` TIMESTAMP NOT NULL COMMENT '最近一次登录时间（时间戳）' ,
...
CREATE  TABLE IF NOT EXISTS `blog`.`blog_role` (
  ...
  `status` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '启用状态：0:表示禁用；1:表示启用' ,
  `remark` VARCHAR(45) NOT NULL COMMENT '备注信息' ,
  PRIMARY KEY (`role_id`) )
ENGINE = MyISAM
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_general_ci
...
CREATE  TABLE IF NOT EXISTS `blog`.`blog_node` (
  ...
  `name` VARCHAR(20) NOT NULL COMMENT '节点名称（英文名，对应应用控制器、应用、方法名）' ,
  `title` VARCHAR(45) NOT NULL COMMENT '节点中文名' ,
